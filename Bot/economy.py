...
class Economy(commands.Cog):
    ...
    async def beg(self, ctx):
        ...
        amount = random.randrange(25, 150)

        outcomes = [
            f"You got **{amount}**",
            f"Batman gave you **{amount}**",
            f"You begged your mom for **{amount}**",
            f"Here's **{amount}**! Be sure to declare that income on your taxes!"
            f"You took a big ol' crap and found **{amount}** in the mess!",
    ...
    async def dep(self, ctx, amount):
        ...
        if amount == "all" or amount == "max":
            sql = "UPDATE main SET wallet = ? WHERE member_id = ?"
    ...
    async def withdraw(self, ctx, amount: str):
        ...
        if amount == "max" or amount == "all":
            ...
            await ctx.send(
                f"You successfully withdrawed **{result[2]}** to your wallet!"
            )
            done = True
        ...
        if not done:
            ...
            if amount >= result[2]:
                ...
                add_bal(ctx.author, amount)
                await ctx.send(
                    f"You successfully withdrawed **{amount}** to your wallet!"
                )
    ...
    async def daily(self, ctx):
        ...
        amount = 6000
        add_bal(ctx.author, amount)
        await ctx.send("**6000** Was placed in your account!")
    ...
    async def work(self, ctx):
        ...
        outcomes = [
            ...
            f"You got a promotion! You earned **{amount}** today."
        ]
